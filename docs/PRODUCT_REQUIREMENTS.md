...
1. 打开页面时，先按浏览器语言或已保存偏好显示中文/英文，并进入“我的房间 / My Room”大厅；默认只展示 `19 × 19` 竹筒棋盘的 120° 弧面预览，不自动开始棋局。
2. 大厅的主要动作必须明确写成“邀请对局 / Invite Game”。用户从同一邀请弹层选择本地双人、本地 AI、AI 自对弈或好友在线，并在真正开局前配置拓扑、宽度、高度、计分规则、贴目、计时和 controller。
3. 本地选择在最终确认后建立 match session；好友在线只在邀请确认后创建后端房间并生成邀请链接，受邀者通过该链接加入即视为接受邀请，双方到齐后才允许开始行棋。
4. 实际建立棋局后，侧栏自动回到“棋局”；通过 OGS 风格的同一组标签在棋局、AI 分析、聊天、棋谱和“棋盘设置”间切换，棋盘始终占据主视觉区。
...
- 一局结束后统一显示“直接进行下一局 / Start Next Game Now”和“调整下一局设置 / Adjust Next-Game Settings”两项终局入口，不在终局操作区另设“退出房间”。“直接进行下一局”按上一局完全相同的拓扑、宽高、规则、贴目、计时、controller 与对手配置立即建立新局；“调整下一局设置”只进入预填上一局参数的设置确认阶段，不得立即建立棋局，只有点击“使用当前设置开始下一局”后才建立新局。
- `transport=local` 时，设置确认阶段必须保留上一局的 controller 与对手上下文，包括本地双人、AI 模型、玩家执子颜色和 AI 自对弈模式；`transport=online` 时仅黑方房主可以编辑并确认下一局，房间号、双方席位、自动 AI 席位、观众和聊天记录全部保留。控制器是人类还是 AI 不产生第二套设置或新局流程。
...
~~~ts
type MatchLifecycle = "lobby" | "waiting" | "playing" | "finished";
~~~

- 首次加载固定进入 `lobby`，标题和主要状态为“我的房间 / My Room”。此时不存在可操作的 live match session；渲染器可以用下一盘设置草稿生成棋盘预览，但该预览不拥有手数、轮次、Replay、AI 任务或权威计时，也不得被序列化成一盘棋。
- 大厅内唯一的主要动作名称是“邀请对局 / Invite Game”，不得写成“开始对局 / Start Game”，也不得用一个不起眼的“AI 对战”链接在点击后立即落入“对局中”。
- 邀请弹层复用同一份新局设置和校验，并至少提供四个明确入口：本地双人、本地 AI、AI 自对弈、好友在线。前三者最终确认后进入 `playing`；好友在线确认后先进入 `waiting`，创建后端房间并展示可复制链接/房间码。
- 受邀者通过有效邀请链接以玩家身份加入即视为接受邀请，不再要求第二个重复的“接受”按钮。在线房两位棋手未到齐时保持 `waiting`；到齐后才进入 `playing`、启动权威时钟并允许第一手。
- `lobby` 和 `waiting` 中，棋盘仍可切换视图、旋转、缩放和平移，用于理解目标拓扑；任何左键/轻点都不能落子，也不能显示可落子幽灵棋、播放落子音或推进状态。停一手、悔棋、认输、点目、复盘播放、AI 分析及棋局计时必须隐藏或明确禁用。
...
- 页面进入 `lobby` 不得自动请求房间 API、建立 WebSocket、加载 AI 模型或启动 Worker/计时器。只有用户确认对应邀请后，才创建其确实需要的资源；关闭或取消邀请必须保持在原大厅且零副作用。
...
- 打开应用和进入本地“我的房间”大厅都不访问房间 API，也不预建一个空 Durable Object。只有用户在“邀请对局”中选择好友在线并最终确认后，才能调用创建房间接口；取消弹层、修改草稿或只旋转预览均保持零网络副作用。
...
- 好友在线邀请在创建成功后生成带房间定位信息的可分享链接；链接不得包含任何创建者或受邀者的 session token。受邀者打开链接并以 `role:"player"` 成功加入即构成邀请接受，服务端分配白席并广播席位变化；不再另存一份易失步的“邀请已接受”状态。若链接无效、房间过期或白席已占用，必须显示明确结果且不能误入可落子状态。
...
“棋盘设置”尤其只能有一份表单 DOM：拓扑、宽高、规则、贴目和计时都从同一表单读写。它在 `lobby` 中是下一局的可编辑草稿并实时驱动预览；进入 `waiting/playing` 后全部锁定，不能通过修改字段或顶部拓扑按钮绕过邀请流程重开。进入 `finished` 后，本地设备或在线黑方房主点击“调整下一局设置”，或者直接打开“棋盘设置”，都必须进入同一个预填设置确认阶段并重新开放表单；共享卡片使用“设置并发起下一局 / 使用当前设置开始下一局”，在线白方和观众仍只读。controller/transport 在同一邀请流程的最后一步选择；切换选项不得丢失草稿、重复 ID/事件监听或造成两个互相不同步的隐藏表单。
...
8. 大厅切换拓扑会立即重建空白预览并切到该形状最直观的视图；`playing` 中拓扑和宽高全部锁定。终局点击“直接进行下一局”会按完整旧配置立即建立空白新局；点击“调整下一局设置”则显示预填上一局全部参数的空白预览，确认前不建立新局，修改为新拓扑或尺寸并确认后必须按新邻接建立全新棋局。
...
6. 确认三种本地邀请后才进入 `playing` 并解锁对应棋局操作；确认好友在线后才发送一次建房请求并进入 `waiting`，此时仍不能落子或启动时钟。
7. 第二浏览器打开邀请链接并成功以玩家加入后，即视为接受邀请；两个客户端进入同一权威棋局且第一手/时钟才可用。链接不含 token，过期、无效或已满时均不能误进入 playing。
...
4. **浏览器交互测试**：浏览器语言选择/持久切换、大厅零副作用、邀请弹层四入口、好友链接接受、实际开局前禁用棋局动作，以及真实 Canvas pointer/touch、拖动阈值、resize、侧栏/时钟、transport × 已支持 controller 组合、唯一 DOM、聊天本地引导、房主浏览器自动白方、观战本地分析、候选 hover/focus/pin、视图切换和 reduced motion。现有 Node 测试不能替代这一层。
...
| 终局新局操作 | 直接同配置或预填后确认 | 保留模型/执子 | 保留自对弈模式 | 仅黑方房主操作 | 保留自动白席 | 等待房主、只读 |
