...
“棋盘设置”尤其只能有一份表单 DOM：拓扑、宽高、规则、贴目和计时都从同一表单读写。它在 `lobby` 中是下一局的可编辑草稿并实时驱动预览；进入 `waiting/playing/finished` 后全部锁定，不能通过修改字段或顶部拓扑按钮绕过邀请流程重开。controller/transport 在同一邀请流程的最后一步选择；切换选项不得丢失草稿、重复 ID/事件监听或造成两个互相不同步的隐藏表单。
...
8. 大厅切换拓扑会立即重建空白预览并切到该形状最直观的视图；`playing` 中拓扑和宽高全部锁定。终局后先“返回房间设置新棋局”，再通过邀请流程用新拓扑开局，不得只换几何却保留按旧邻接形成的棋局。
