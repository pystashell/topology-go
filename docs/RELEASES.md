...
1. `public/version.json` 中的 SemVer，例如 `0.2.0-rc.3`；
2. 指向唯一 Git commit 的 Git tag，例如 `v0.2.0-rc.3`；
...
```powershell
npm.cmd run release:plan -- 0.2.0-rc.3
```
...
npm.cmd run release:version -- 0.2.0-rc.3
...
```powershell
npm.cmd run release:check -- 0.2.0-rc.3
```
...
git status --short
git rev-parse HEAD
npm.cmd run release:publish -- 0.2.0-rc.3
```
