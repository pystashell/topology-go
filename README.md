...
- 在线房间支持邀请链接、旁观、服务端权威判定、双方点目确认、协商悔棋、服务端认输判定、断线回座与 24 小时无活动清理；一局结束后可以“直接进行下一局”，按完全相同的棋盘、规则、计时与对手配置立即开局，也可以选择“调整下一局设置”，进入预填上一局参数的同一棋盘设置卡片，修改后点击“使用当前设置开始下一局”。在线仅黑方房主可以执行两项新局操作，房间号、双方席位、AI 对手、观众和聊天记录都会延续；本地 AI、自对弈和本地双人采用相同界面与流程并保留上一局对手上下文
